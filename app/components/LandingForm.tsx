...
// DEBUG: temporary overflow scanner + overlay — remove before committing
useEffect(() => {
  const overlayId = "overflow-debug-overlay";

  function scan() {
    const vw = window.innerWidth;
    const all = document.querySelectorAll("*");
    let worst: { tag: string; className: string; width: number; overflow: number } | null = null;

    all.forEach((el) => {
      if ((el as HTMLElement).id === overlayId) return;
      ...
      if (rect.right > vw) {
        ...
        if (!worst || over > worst.overflow) {
          worst = {
            ...
            className: (el as HTMLElement).className?.toString().replace(/\s+/g, ".").slice(0, 60) || "",
    ...
    if (worst) {
      const w = worst as { tag: string; className: string; width: number; overflow: number };
      overlay.innerHTML = `<strong>Largest overflow:</strong><br>${w.tag}${w.className ? "." + w.className : ""}<br>${w.width}px / ${vw}px<br><strong style="color:#f87171">+${w.overflow}px</strong>`;
    } else {
      overlay.innerHTML = `<strong style="color:#86efac">No overflow detected</strong><br>vw: ${vw}px`;
    }
  }
