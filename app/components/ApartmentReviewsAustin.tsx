...
export default function ApartmentReviewsAustin() {
  /* ================================
     CITY-SPECIFIC DATA (ONLY CHANGES)
     ================================ */
 ...
 const matchQuery = matchReviewQuery(q, listing.property_name);
  ...
  const matchBaths =
    numericBaths === null ||
    (numericBaths >= listingMinBaths &&
      numericBaths <= listingMaxBaths);

  // ✅ NEIGHBORHOODS (ROBUST MATCH)
...
const matchNeighborhoods =
  ...
  neighborhoods.some((n) =>
    normalize(listing.neighborhood || "") === normalize(n)
  );
  ...
  const matchSubmarkets =
    ...
    submarkets.some((s) =>
      listing.submarket?.toLowerCase().includes(s.toLowerCase())
    );

  // ✅ PROPERTY TYPE LOGIC (IDENTICAL)
