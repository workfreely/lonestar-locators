...
export async function GET(req: NextRequest) {
  ...
  const formattedMoveInDate =
    rawMoveInDate && !isNaN(new Date(rawMoveInDate).getTime())
      ? new Date(rawMoveInDate).toLocaleDateString("en-US", {
          month: "long",
          day: "numeric",
          year: "numeric",
        })
...
<p style="font-size:16px;line-height:1.7;color:#333;">
  ${firstName}, you selected ${
    incentive === "cash"
      ? "<strong>a cash rebate</strong>"
...
${
  formattedMoveInDate
    ? `
<p style="font-size:16px;line-height:1.7;color:#333;">
  Your move-in date is <strong>${formattedMoveInDate}</strong>.
</p>
`
    : ""
}
...
`
    : `
<p style="font-size:16px;line-height:1.7;color:#333;">
  Your rebate will be processed and issued within <strong>90 days of your move-in date.</strong>
</p>
`
}
