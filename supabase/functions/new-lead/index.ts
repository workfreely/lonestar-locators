...
<tr>
<td style="background:#1f8f4a;padding:18px 24px;color:#ffffff;">
<h2 style="margin:0;font-size:20px;">Lone Star Locators</h2>
<p style="margin:4px 0 0;font-size:13px;">New renter lead notification</p>
</td>
</tr>
...
<h2 style="margin-top:0;">New Renter Lead: ${fullName}</h2>
...
">
  ...
  ${pill(
    "Move",
    lead.move_date
      ? `${formatDate(lead.move_date)} (${window})`
      : null
  )}
  ...
  ${pill(
    "Credit",
    lead.credit_history
      ? `${lead.credit_history}${lead.credit_score ? ` (${lead.credit_score})` : ""}`
      : null
  )}
  ${pill("City", lead.city)}
...
`
    ...
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Jay @ Lone Star Locators <onboarding@resend.dev>",
