...
type InvoiceForm = {
  ...
  invoice_expected_commission: string
...
function seedForm(lead: any): InvoiceForm {
  ...
  return {
    ...
    invoice_expected_commission: num(lead.invoice_expected_commission),
...
export default function InvoiceDetailsSection({
  ...
  // Total Invoice Amount = Base Rent × Commission % (auto, live). Example:
  // $1,500 × 150% = $2,250. Read-only — derived, never stored.
  ...
  return (
    <SectionCard title="Invoice Details" collapsible defaultOpen={false} storageKey="invoice-details">
      ...
      {/* Total Invoice Amount — auto-calculated (read-only) */}
      <div className={`${rowCls} pt-0.5`}>
        <span className={labelCls}>
          Total Invoice Amount
          <span className="ml-1 text-[10px] text-[var(--crm-text-muted)] normal-case">(auto)</span>
        </span>
        <span className="text-[12.5px] font-bold text-[var(--crm-text-primary)] text-right">
          {totalInvoice === null ? "—" : formatCurrency(totalInvoice)}
        </span>
      </div>

      {/* Expected Commission — manual for now */}
      <div className={rowCls}>
        <label className={labelCls}>Expected Commission ($)</label>
        <input
          type="number"
          min="0"
          step="0.01"
          className={inputCls}
          value={form.invoice_expected_commission}
          onChange={(e) => setForm((f) => ({ ...f, invoice_expected_commission: e.target.value }))}
          onBlur={() => saveNumber("invoice_expected_commission")}
          placeholder="—"
        />
      </div>
